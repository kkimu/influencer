...
int_t *get_influencers_list(varNode *Node, int_t N, int_t **Graph, int L, int_t DECIM_STEP, const char *OUTFILENAME) {
	...
	FILE *list_inf = fopen(OUTFILENAME, "w");
...
int main(int argc, char *argv[]){
	...
	int L;
	const char *network, *output;
		if (argc != 4) {
		fprintf(stderr, "usage: %s <NETWORK_FILENAME> <L> <OUTPUT_FILENAME>\n", argv[0]);
		exit(1);
	}
	network = argv[1];
	L = atoi(argv[2]);
	...
	varNode *Node = (varNode *)calloc(N + 1, sizeof(varNode));
	...
	int_t *list_influencer = get_influencers_list(Node, N, Graph, L, DECIM_STEP, output);
