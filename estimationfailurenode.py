...
# k-coreで推定したランキングをkc_rankに入れる
kc_rank = []
kc = defaultdict(int)
kc_path = "analysis/{}_kcore.list".format(dataset)
for line in open(kc_path):
    sp = line.strip().split(" ")
    kc_rank.append(sp[0])
    kc[sp[0]] = sp[1]
...
for node in failednodes:
    ...
    out.append("{},{},{},{},{},{},{},{},{},{},{},{}\n".format(node,rt_ranki,deg_ranki,pr_ranki,clo_ranki,bet_ranki,kc_ranki,degree,pagerank,closeness,betweenness,kcore))
        
with open("analysis/{}_failednodes_top{}.csv".format(dataset,top),"w") as f:
    f.write("node,correct rank,degree rank,pagerank rank,closeness rank,betweenness rank,k-core rank,degree,PageRank,closeness,betweenness,k-core\n")
    f.writelines(out)
